	...
	public void startApp() {
		while(true) {
			...
			if(view.action == 1) { // 메뉴판
				...
				int num = view.inputInt(); // 입력받은 값을 num에 저장
				vo.setNum(num); // num 값을 임시객체 vo에 담아서
				...
				while(true) { // 관리자모드는 종료시점까지 반복
					...
					if(view.action == 1) { // 관리자 1번 메뉴추가파트
						...
						String name = view.inputString(); // name에 입력받은 이름 저장
						view.inputPrice(); // 가격 입력 안내
						int price = view.inputInt(); // price에 입력받은 가격 저장
						view.inputCnt(); // 재고 입력 안내
						int cnt = view.inputInt(); // cnt에 입력받은 재고 저장
						PopcornVO vo = new PopcornVO(); // 임시객체를 만들어주고
						vo.setName(name); // 임시객체.name에 입력받은 이름 대입
						vo.setPrice(price); // 임시객체.price에 입력받은 가격 대입
						vo.setCnt(cnt); // 임시객체.cnt에 입력받은 재고 대입
						...
						int num = view.inputInt(); // num에 입력받은 번호 저장
						view.inputCnt(); // 재고 입력 안내
						int cnt = view.inputInt(); // cnt에 입력받은 재고 저장
						PopcornVO vo = new PopcornVO(); // 임시객체 생성
						vo.setNum(num); // 임시 객체.num 에 입력받은 번호 대입
						vo.setCnt(cnt); // 임시 객체.cnt 에 입력받은 재고 대입
						...
						int price = view.inputInt(); // price에 입력받은 가격 저장
						PopcornVO vo = new PopcornVO(); // 임시객체 생성
						vo.setNum(num); // 임시객체.num 에 입력받은 번호 대입
						vo.setPrice(price); // 임시객체.price에 입력받은 가격 대입
						...
						int num = view.inputInt(); // num에 입력받은 번호 저장
						PopcornVO vo = new PopcornVO(); // 임시객체 생성
						vo.setNum(num); // 임시객체.num에 입력받은 번호 대입
